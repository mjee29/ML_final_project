...
# 추천 함수 정의
def recommend_perfumes(user_input, top_n=10):
    # 사용자 입력 벡터화
    user_vector = vectorize_category(user_input)

    # 코사인 유사도 계산
    data['Similarity'] = data['Vector'].apply(lambda x: cosine_similarity([x], [user_vector])[0][0])

    # 유사도가 높은 상위 n개 추천
    recommended_perfumes = data.nlargest(top_n, 'Similarity')

    return recommended_perfumes[['Name', 'Similarity', 'category']]

# Streamlit 인터페이스 구축
st.title("Perfume Recommendation System")
st.write("Select categories from the following options:")
categories = [
...
if st.button("Recommend"):
    ...
    recommended_perfumes = recommend_perfumes(user_input, top_n=10)
    st.write("\nTop 10 recommended perfumes:")
    st.write(recommended_perfumes)
